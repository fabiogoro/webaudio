...
function noise_block(top, bottom, size){
  ...
  var bottom_bin = Math.round(bottom*size/audio_context.sampleRate);
  var top_bin = Math.round(top*size/audio_context.sampleRate);
  var norm = 0;
  ...
  return f.fromComplexArray(res);
}

function noise_gen(top, bottom){
  const size = 65536;
  var real = new noise_block(top,bottom,size);
  var real2 = new noise_block(top,bottom,size);
  var bufferSize = size;
  ...
  for (var i = 0; i < size/2; i++) {
    output[i] = (real[i]+real2[size/2+i]);
    output[i+size/2] = (real[i+size/2]+real2[i]);
  }
