function Noisy(audio_context){
  this.audio_context = audio_context | create_context();
  this._size = 65536;
  this._fft = new FFT(this._size);
}

Noisy.prototype._noise_block = function _noise_block(top, bottom){
...
Noisy.prototype._noise_gen = function createNoise(top, bottom){
  const size = this._size;
  const audio_context = this.audio_context;
